...
## 日報のテンプレートを作りスタッフがそれに沿って記入し日報を提出できる日報管理システムです

次の画像はNipoPlusで日報を読んでいる画面のスクリーンショットです。

{{<icatch filename="app" msg="NipoPlus実際の画面 一人承認した日報の例" title="NipoPlusのアプリ画面のイメージ図" fontsize="30px" alice="ok" >}}

「A品質・B品質」など色々な項目が目に付きますね。これらの項目は日報テンプレート作成時に自由にカスタマイズが可能です。
つまりどんなにマイナーな日報であっても自分たちで自由にカスタマイズができる日報アプリなのです。
