...
menuTitle = "選択肢入力"
tags = []
title = "日報テンプレートに選択式の入力フォームを追加する"
toc = true
weight = 120
+++
...
日報作成者から見れば回答が用意されていて選ぶだけで済むため入力業務の負担が大きく軽減できるメリットがあります。
日報を読む側から見れば表記ゆれや想定外の文字を入力されることがないため一定の品質を保った日報になるため閲覧の負担が軽減できます。
PCではクリック、[タブレット](/blog/ipad/)や[スマートフォン](/blog/smartphone/)ではタップ操作だけで簡単に入力ができます。選択の項目も自由に設定が可能です。
選択肢に色付けをすることで視認性を向上させることも可能です。色については本ページの「選択肢の色分けについて」のセクションを参照してください。
選択肢で選ばれた解答は[集計](/report/totalling/)することも可能です。
