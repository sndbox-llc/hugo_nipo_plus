+++
description = "現場で作成された日報をそのままサクッと集計OK。エクセルで加工するもよし。簡単にアプリ上から集計やグラフ📊を作るもよし。KPI目標設定なども可能です"
...
{{<icatch filename="kouji" msg="稼働人員や稼働時間 数値データを集計する" title="集計対象となる日報の例。文字は集計できないが数値は集計できる" fontsize="30px" alice="here">}}
...
### 数百枚の日報を1枚のエクセルシートに一瞬でまとめることが可能です。集計の準備に最適
...
{{<btnCenter "/excel/" "エクセルを使った日報集計テクニック">}}

### 集計の元データをより使いやすい形で出力可能です。用途に応じてCSV出力オプションを設定
...
{{<icatch filename="anken" msg="案件登録された日報 案件別の集計が可能" title="集計対象となる日報の例。文字は集計できないが数値は集計できる" fontsize="30px" alice="here">}}
...
次のような部品の生産を報告する日報（伝票）を用意しました。  

{{<appscreen filename="make-receipt" title="部品の生産個数を入力するだけの伝票のような日報。もはや伝票と表現したほうが適切である">}}
...
{{<imgproc kpi-1.webp "製造数は日報から自動で吸い上げてKPI表に反映されます。日々の進捗を確認し遅れがあれば早期に対応するなどの判断が可能です" />}}
...
<!--
グラフ生成の関係上、集計項目を増やす必要が有りましたので例題を変更します。次のような日報を元に集計をしてみましょう▼


{{<icatch filename="report1" msg="国・社・数・理・英 5科目を集計しよう" title="集計対象となる日報の例。文字は集計できないが数値は集計できる" fontsize="30px" alice="here">}}
