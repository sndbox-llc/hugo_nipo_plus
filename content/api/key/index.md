...
{{<imgproc apikeyGet.png "APIキーを表示する" />}}
...
{{<imgproc invoke.png "APIキーの破壊をクリックしてAPIキーを無効化できます。この操作と同時にログアウトが自動で行われます" />}}
...
{{<imgproc disablestaff.png "特定のスタッフアカウントを無効化することでそのスタッフのAPIキーも含めて無効化されます" />}}
