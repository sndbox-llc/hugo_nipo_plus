...
|2022/09/14|v1.13.1|[過去データ推移](/report/read/analytics/)の色分け機能にバグがあったため修正を行いました。（すでに設定済みの場合、設定はクリアされます）|○|○|○|
|2022/09/13|v1.13.0|[過去データ推移](/report/read/analytics/)で最大取得件数を10件から30件に拡張しました。（オプションから選択します）過去データ推移で各行ごとに任意の色付けをすることが出来るようになりました。スタッフのアカウントを無効化するとAPIキーも同時に破壊されるように修正しました。手動でAPIキーを破壊できる機能を追加しました。|○|○|○|
