...
{{<appscreen filename="report-delete" title="日報の一括削除" desc="一括削除では承認済みの日報や進行中の日報もお構いなしに削除できます。実行には編集者権限が必要です" >}}
...
{{<appscreen filename="search-save" title="検索条件の保存" desc="検索の条件を保存することで次回から同じ条件の検索をワンクリックで行なえます" >}}
