...
{{<appscreen filename="anken-add" title="案件を追加する">}}
...
{{<appscreen filename="set-anken" title="レポート作成時に案件をレポートに紐づけします">}}
...
{{<appscreen filename="read-report-anken" title="案件に紐づけされたレポートを表示した画面の例。通常のレポートとほとんど変わりませんが、ヘッダー右側に紐づけされた案件名が記載される点が違います">}}
...
{{<appscreen filename="anken-list" title="案件一覧表">}}
...
{{<appscreen filename="anken-detail" title="案件の詳細">}}
...
{{<imgproc anken-sumtotal.webp "案件の集計画面。列見出しに案件名が入り、各行はそれぞれの案件で入力された数値が集計されて入ります。累積を見たい場合に便利な機能です。" />}}
