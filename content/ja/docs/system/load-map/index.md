...
{{<warning>}}
2023年7月追記:開発スケジュールに遅れが生じています。更新に時間がかかっておりご不便をおかけして申し訳ございません。
{{</warning>}}
...
登録日|内容|顛末
---|---|---
2023/08/03|日報作成と同時にタイムカードの作成ができるような機能の実装（リクエスト）|
2023/07/28|日付入力時、特定の環境下で日付が自動でにゅうりょくされてしまうケースへの対応（バグ）|
2023/07/20|日報テンプレートの階層分けを実装する（フォルダのように分類分けがしたい）（リクエスト）|
