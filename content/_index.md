...
nipoplusは簡単な操作でオリジナルのテンプレートを作成できるクラウド型のアプリケーションです。
数値や写真、選択肢など、様々な入力フォームを組み合わせて自由自在にテンプレートをカスタマイズできます。
...
## データの分析とBI化（グラフや表に変換）で業務効率UP

日報や日誌、チェックシートなどのカスタマイズ可能なテンプレートを通じて保存されたデータの収集を簡単かつ効率的に行うことができます。  
（※本文や写真など集計対象外のデータは集計が行われません）  

データの分析には大きくわけて2種類の方法を提供しています。1つはデータのCSVエクスポート。CSVで出力することでエクセルなどの表計算ソフトを使ってデータを自由に集計、分析できます。  
...
{{<button "/tips/smartphone/" "スマホ・タブレットの日報作成について">}}
...
{{<button "/tips/checksheet/" "日報の集計方法について詳しく見る">}}

{{<button "/tips/sales_report/" "応用集計について">}}
