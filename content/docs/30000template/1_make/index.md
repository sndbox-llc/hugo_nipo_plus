...
## テンプレートとはデータ入力枠のことです{#template_definition}

ひな型やフォーマットと呼ばれることもありますが、このサイトでは「テンプレート」と呼びます。
テンプレートとはデータを入力するための入力フォーム（入力枠）のことです。例えば日付の枠や作業内容を書く枠など、日報に必要な入力枠を自由に組み合わせてオリジナルの日報テンプレートを簡単に作成することができます。

一度テンプレートを作ってしまえば、日報を書く際、作成したテンプレートに沿って入力するだけで良いため、記入漏れのリスクも削減でき、また誰が書いても一定の品質を保った日報が作成可能になります。

## オリジナルの日報アプリを自分たちで作る

{{<icatch filename="img/icatch" msg="部品を組み合わせてテンプレートを作ろう" alice="ok">}}

日報アプリにおいて重要なものはテンプレートです。  
会社によって日報のテンプレートは様々。同業他社であっても「日報のテンプレートは全く違う」なんてこともよくあります。  
多くの企業は日報にWordやエクセルを使用しています。これは前述の通り、会社によってテンプレートが全く違うため、なかなかシステム化しにくいという点も大きな理由の一つでしょう。  
NipoPlusではプログラミングの専門知識がなくても、会社オリジナルの日報テンプレートを簡単に作成することが可能です。

入力フォームと呼ばれる様々な部品を、ブロックのように組み合わせていくだけで簡単にオリジナルの日報テンプレートが仕上がります。  
入力フォームは10種類以上ありますので、用途に合わせて自由にチョイスして組み合わせて下さい。

テンプレートは何種類でも必要に応じて作成できます。  
もちろん日報に限らず、月報や、設備の点検表としても利用可能。
