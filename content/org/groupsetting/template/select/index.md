...
{{<imgproc csvsetting.png Resize "1200x" "CSV出力前の設定を変更する。選択肢の場合は列展開のON/OFFにより出力形式が変化します" />}}
...
{{<imgproc csv.png Resize "1200x" "上のシートが展開ON・下のシートが展開OFFの設定でそれぞれCSV出力したもので、それらをエクセルで開いた画面です。" />}}
...
{{<imgproc calc.png Resize "1200x" "NipoPlusの集計機能を使ってチェックシートの選択肢データをスタッフごとに自動で集計します" />}}
