...
- 「承認して次へ」を押すと日報の一覧の色が反映されないバグを修正しました
- 一括PDF出力で「検印欄」の出力設定が正しく反映されないバグを修正しました
- カレンダー選択で「日付特定に失敗」と表示されるバグを修正しました
- 日報受信BOXで指定した期間以外の日報が起動直後に１回表示されるバグを修正しました
- 日報の綴り機能が実装されました
...
#### {{<nipoPlusLogo 30>}} 2025/11/１4 v1.75.1{#vp1_75_1}

- 日報を承認するとフリーズするバグを修正しました
- 日報の共有リンクを作成するとフリーズするバグを修正しました

ご不便をおかけして申し訳ございません。Version 1.75.0をご利用の方は早期にこのバージョンに移行をお願いします。

#### {{<nipoPlusLogo 30>}} 2025/11/１4 v1.74.3へ一時的にロールバック

ご不便をおかけして申し訳ございません。先日リリースされたVersion 1.75.0において、日報の承認や共有リンク作成など、特定の通信処理を行うとフリーズするバグが確認されたため、一時的に一つ前の安定バージョンにロールバックしました。
原因について現在調査中です。現在はロールバックされておりVersion 1.74.3がリリースされております。

#### {{<nipoPlusLogo 30>}} 2025/11/１３ v1.75.0{#vp1_75_0}

- 顧客メモに対しても部外者共有リンクが作成できるようになりました
- 反復入力フォームでドラッグによる幅変更ができるようになりました
- 名称を「レポート」から「日報」に変更しました
- カレンダーの最大読み込み件数をプランに応じて変更するように修正しました
- 集計機能の最大読み込み件数をプランに応じて変更するように修正しました
- カレンダー・集計機能が無料プランでも利用できるようになりました

#### {{<nipoPlusLogo 30>}} 2025/11/4 v1.74.3{#vp1_74_4}

- 目標実績で特定の環境下でデータの保存ができないバグを修正しました
- スマートフォンでキーボード表示中はフッターを非表示にするように微調整を行いました

#### {{<nipoPlusLogo 30>}} 2025/10/３1 v1.74.3{#vp1_74_3}

- スマートフォンアプリでFooterとの干渉により提出ボタンが押せないケースが有る問題を修正しました

本アップデートはAndroid／IOS版でのみリリースされます。Web版は次回まとめてリリース予定です

#### {{<nipoLogo 30>}} 2025/10/21 v11.0.0{#legacy11_0_0}

- 内部ライブラリの大規模なアップデートを実施しました

本アップデートによる機能の追加はございませんが、内部的に大規模なアップデートを実施しました。
具体的には、Nipoの内部で使用しているFirebaseSDKというプログラムを8シリーズから10シリーズにアップデートしました。
これにより今後も中長期的に安定してメンテナンス等が行える環境になりました。

#### {{<nipoLogo 30>}} 2025/10/21 サーバサイドアップデート

- 旧バージョンのNipoにおいて、サーバサイドの定期メンテナンスアップデートを実施しました

※本アップデートは保守のため、機能追加や機能変化はございません。
...
- ビルド方式をModule式にアップデート。今どきの開発環境に合うように抜本的な改善がなされました。

今回のアップデートによる機能の追加などは有りませんが、より現代風のソースコードになったことで保守、管理の手間が大幅に向上しました。
