...
- テンプレート編集で幅が反映されないバグを修正しました


#### {{<nipoPlusLogo 30>}} 2024/7/29 v1.59.1{#vp1_59_1}

- 長年メンテナンスされていないドラッグ操作のプログラムコードをNipoPlusから取り除きました
- テンプレート編集時、項目の並べかえを複数選択でまとめて行えるようになりました
- 上記に関連して、並べ替え処理はすべて複数選択ができるように修正されました
- CSV出力時、文書番号のまえに文書番号プレフィックスもセットで出力されるようになりました
- ドラッグで幅変更ができるテーブルで、スマートフォンやタブレットのタッチイベントでも幅変更できるように修正しました
- 日毎カレンダー表示で１時間毎の水平線が追加されました


#### {{<nipoPlusLogo 30>}} 2024/7/22 v1.58.0{#vp1_58_0}

- CSV出力に「文書番号」出力オプションが追加されました
...
- 写真入力フォームに添付する画像を無料プランでは低画質化させました（容量節約のため無料プランの方には申し訳ございません）
- 反復入力内に日付と時刻フォームでカレンダーを表示するとレイアウトが崩れるバグを修正しました 
- レポートのロード件数を有料プラン限定で上限1000件に引き上げました
