...
{{<appscreen filename="readed" title="レポートを既読する。既読は承認できないスタッフがレポートを読んだことを伝えるための機能です">}}
...
{{<appscreen filename="report-approval" title="レポートの状態が承認になる">}}
...
{{<appscreen filename="report-rejected" title="承認リレー中に誰かが棄却するとその時点でレポートの状態は「棄却」となります。">}}

棄却の際は承認の権限が次のスタッフへ移動しません。そのため承認や棄却ボタンはそのまま表示され続けます。
棄却された事実は**レポートの作成者に通知**されます。棄却されたレポートをその後どうするのかは各会社でルールを明確に決めておくと良いでしょう。
...
{{<appscreen filename="report-fixed" title="修正されたレポートは承認のハンコがクリアされます">}}
