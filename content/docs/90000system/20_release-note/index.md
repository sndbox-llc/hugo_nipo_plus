...
- 顧客メモからレポート保存箱へ直接移動した場合に限り一部のデータ初期化が機能していないバグを修正しました
- レポート提出状況を「ユーザ指定日付」以外に「サーバ保存日」基準で表示できるように機能が追加されました
...
- レポートを棄却時に「棄却理由」を記述するポップが表示されるようになりました。この機能はGOLDPLAN限定です。棄却理由はコメント欄に追記されます。
...
- 語録に「レポート作成時に単語の追加を許可する・許可しない」のオプションが追加されました
