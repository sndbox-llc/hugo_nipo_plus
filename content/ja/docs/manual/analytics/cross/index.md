...
{{<appscreen filename="table-make" msg="異なるテンプレート同士を同時に集計するクロス集計を作成する">}}
...
{{<appscreen filename="table-edit" msg="行と列とセルをそれぞれ設定する">}}
...
{{<appscreen filename="calc-field" msg="クロス集計に計算列を追加">}}

### 結果のプレビューと保存{#save}

実際の集計結果はプレビューで確認できます。

{{<appscreen filename="previews" msg="クロス集計の結果をプレビューで表示">}}
...
{{<appscreen filename="view" msg="作成したクロス集計を表示する">}}
