...
{{<icatch filename="eye-catch" msg="選ぶだけ！簡単入力" title="選択式入力フォームを含んだテンプレート" fontsize="30px" alice="here">}}

## 概要

あらかじめ回答のリストを用意しておき、日報作成時や設備の定期点検時には一覧から選ぶだけで入力ができる形式のフォームです。  
「キーボード不要の入力」で、スピーディな入力が可能です。  
見た目も非情に一般的で、コンピュータに不慣れな方や年配の方でも直感的な入力が可能です。  
また、その性質上スマートフォンやタブレットといったタップ操作に対応した端末とも相性の良いフォームです。
...
1. テンプレート編集画面を開く
1. フォームリストから「選択入力系」のどれかをクリックして追加
...
### チェックボックスのデータをCSVに出力{#csv_checkbox}
...
例えば「地域」という[選択式入力フォーム](/docs/manual/initial-setting/template/selects/#plain)があったとします。
