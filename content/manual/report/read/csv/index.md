...
現在画面に表示されている**日報1件をCSVファイルに変換してダウンロード**できます。（複数まとめて出力したいときは[一括CSV出力](/manual/report/analytics/csv/)を参照)
...
また、ここで設定したCSV出力設定は[一括CSV出力](/manual/report/analytics/csv/)でも同じ設定が自動で適用されます。（逆もまた同様です）
