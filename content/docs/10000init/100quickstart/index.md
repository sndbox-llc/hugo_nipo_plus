...
## 手順4：レポートを書いて自分宛に提出してみよう！ {#write_report}

本来レポートは上司などへ提出するものですが、この時点ではまだ管理者である**あなた一人しかいない**はずです。まずは、自分宛にレポートを作成して一連の流れを見てみましょう。

1.  メニューから「**レポート作成**」をクリック
2.  テンプレート一覧から、先ほど作成したテンプレートをクリック
3.  テンプレートに沿って報告内容を記入します。（練習なので適当な内容でOK）
4.  **提出先を「管理者」（あなたのアカウントです）に設定**し、「**提出**」ボタンをクリック

{{<icatch filename="img/write-report2" msg="自分宛てにレポートを提出する流れを体験してみよう" alice="ok">}}

{{<nextArrow>}}

これでレポートがあなた自身に提出されました。

{{<icatch filename="img/next" msg="レポートを提出した後の画面イメージだよ" alice="book">}}
...
「**作成したレポート表示**」を選んで、先ほど書いたレポートを見てみましょう。

## 手順5：提出されたレポートを承認してみよう！ {#read_report}

「作成したレポート表示」を選ぶと、その[レポートを読む画面](/docs/manual/read-report/state/)へ移動します。
