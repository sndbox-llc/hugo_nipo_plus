...
※このバージョンでレポートが提出できないバグが報告されています。緊急性の高いバグであるため現在はVersion 1.55.1にロールバックさせています
※このバージョンで「期間入力（時刻のみ）」を使い、その項目が入力必須の場合、入力必須の解消が正しくトリガーされずに提出できないバグが報告されています。
※可変長を使い、かつ２行以上のデータがあり、なおかつ下書きからの清書を行うという限定的な条件下でこの現象発生を確認しています。
※一時的にこの項目の入力必須を無効にしていただくことで回避可能です
※最優先でこのバグの修正に着手しています。ご不便をおかけしてすみません。6/26本日中に修正完了の予定です
...
- [メール通知機能](/docs/manual/notice/email/)を実装しました。月50通まで無料です（無料プランは利用不可）。オプション開放で月10,000通まで発信可能になります
